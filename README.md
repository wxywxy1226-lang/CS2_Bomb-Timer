...
一个CS2 炸弹倒计时悬浮窗。CS2 本身不显示炸弹倒计时，本工具通过 Valve 官方 **Game State Integration（GSI）** 接口获知炸弹状态，在炸弹安放后于屏幕显示剩余倒计时，并根据剩余时间提示是否来得及拆弹。
...
- GSI 只在状态变化时携带炸弹字段，且不提供精确秒数；因此本工具在检测到"已安放"的瞬间开始本地计时，按 40 秒推算剩余时间（误差约 0.5 秒）。**一旦安放完成，即使某次推送不包含炸弹数据，倒计时也会持续进行**（粘性计时）；拆除/爆炸、回合结束，或炸弹字段连续 20 秒未再推送（对局提前结束）时复位隐藏。
